	...
	if(aMP == 1){
		switch(aLearnerType){
			case 1: {
				...
				//pierre: convert object aLearner to herited class.
...
int main(int argc, char **argv){
	...
	//入れ子の並列化を許可
	//pierre: Allow nested parallelization
	omp_set_nested(1);
	...
	if(tPE == 1){
		uint tN = tY.size();
		//pierre tK: nb of K in cross validation
		if(tN < tK){
			cout << "error:CV,n<k" << '\n';
			exit(1);
		}
		...
		//index をシャッフル
		//pierre: translate : 'index shuffle'
		vector<uint> tTrainIdx;
		for(uint j = 0; j < tN; ++j){
			tTrainIdx.push_back(j);
		}
		//pierre: rand() % 6 will always produce a result between 0-5.
		//pierre basically this just shuffles training set
		for(uint j = 0; j < tN; ++j){
			uint k = j + (rand() % (tN - j));
		...
		#pragma omp parallel reduction(+:tCorrect)
		{
			//make prefixspan
			...
			for(uint i=0; i < tK; ++i){
				...
				//pierre: taking above example : if we have [4,5,5,5]
				//pierre: we put the first 4 values in test and keep the rest as train etc.
				//pierre: idea ok : we copy test idx in tTestIdx and we delete them from tTrainIdx_private
				...
				//pierre : pas compris le détail mais en gros la partie du haut a pour objectif de créer Xtrain, ytrain. De même en bas.
				//pierre : transaction corresponds to protein seq eg AAATMMMMMZ
				...
				//pierre: initialise graph representation
				tPrefix.init(tTrainTransaction, tTrainY);

				//Learner select 学習機を増やしたらここに追加
				//pierre: here other algorithms can be tried out!
				...
				//calculate lambda sequence on logarithmic scale
