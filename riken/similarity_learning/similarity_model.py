...
class SimilarityModel:
    ...
    def get_triplets_loss(self, inputs, labels):
        ...
        with tf.variable_scope('vector_representation'):
            vectors = self.to_vector_representation(inputs=inputs['tokens'], pssm_input=inputs['pssm_li'])

        with tf.variable_scope('hard_triplet_search'):
            norms = tf.reduce_sum(vectors * vectors, 1)
            ...
            #                  - 2*tf.matmul(a=vectors, b=vectors, transpose_b=True) \
            ...
            p_scal_mat = tf.matmul(a=vectors, b=vectors, transpose_b=True)
            ...
            p_dist = tf.reduce_max(p_dists, axis=0, name='get_hard_pos_dist')
            n_dist = tf.reduce_min(n_dists, axis=0, name='get_hard_neg_dist')
        ...
        # dist_diff = tf.nn.relu(p_dist-n_dist+self.margin*tf.ones(p_dist.get_shape()),
        #                        name='max_with_zero')
        dist_diff = tf.maximum(p_dist - n_dist + self.margin * tf.ones(p_dist.get_shape()), 0,
                               name='max_with_zero')
    ...
    def to_vector_representation(self, inputs, pssm_input):
        ...
        h = tf.concat([embed, aa_static_feat, pssm_input], axis=2, name='all_features')
        ...
        with tf.variable_scope('feature_importances', reuse=True):
            weights = tf.get_variable('kernel')
            print(weights)
            weights = (weights - tf.reduce_min(weights)) / (tf.reduce_max(weights) - tf.reduce_min(weights))
            weights = 255.0*weights
            weights = tf.expand_dims(weights, 2)
            weights = tf.expand_dims(weights, 1)
            tf.summary.image('feature_importances', weights, max_outputs=1)
